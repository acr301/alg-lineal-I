% ======================================================================
%  Informe_Programa 2_Grupo 7.pdf
%  Proyecto Integrador: Calculadora de Álgebra Lineal — UAM (MTM0120)
%
%  Compilar:  pdflatex informe.tex   (dos veces, para el índice)
%  o bien:    latexmk -pdf informe.tex
%
%  Plantilla replicada de "Informe_Programa_1_Grupo_7.pdf" (hecho en Word).
%  Reutilizable entre entregas: sólo cambia la portada (título del Programa),
%  la sección "Explicación del algoritmo" y las 3 capturas de "Casos de prueba".
% ======================================================================
...
% --- Datos de la entrega (editar aquí) --------------------------------
\newcommand{\programaTitulo}{Programa 2: Reducci\'on a Forma Escalonada Reducida (Gauss--Jordan) e Identificaci\'on de Columnas Pivote}
\newcommand{\grupoNum}{7}
\newcommand{\docente}{Jos\'e Andr\'es Mungu\'ia Cort\'ez}
\newcommand{\ciudadFecha}{Managua, \today}
% Integrantes del Grupo 7. NOTA: completar los nombres legales de Roberto y
% Reynaldo (segundo nombre / apellidos) si el formato de la entrega lo exige;
% en el informe 1 se usaron nombres completos.
\newcommand{\integrantes}{%
  Andr\'es Ernesto Castillo Rothschuh\\
  F\'atima Marie Zogaib Gradiz\\
  Roberto Mac\'ias\\
  Reynaldo Molina%
}
% --------------------------------------------------------------------
...
% --- Placeholder para capturas todavía no tomadas --------------------
\newcommand{\captura}[2]{%
  \begin{center}
  \IfFileExists{img/#1}{\includegraphics[width=0.92\linewidth]{#1}}{%
    \fbox{\parbox{0.9\linewidth}{\centering\vspace{1.2em}
      \textbf{[PENDIENTE: captura de pantalla]}\\[0.4em]
      \small #2\\[0.2em]
      Guardar como \texttt{img/#1} (ejecutar \texttt{uv run aqua-gauss}).
      \vspace{1.2em}}}%
  }
  \end{center}%
}
...
\captura{caso1-unica.png}{Caso 1: sistema consistente determinado --- RREF, columnas
pivote 1,2,3 y soluci\'on \'unica $(5,3,-2)$.}
...
\captura{caso2-libres.png}{Caso 2: sistema consistente indeterminado --- RREF,
columnas pivote 1 y 2, $x_3$ libre, soluci\'on vectorial $\mathbf{x}_p + t_1\mathbf{v}_1$.}
...
\captura{caso3-inconsistente.png}{Caso 3: fila $[0\ 0\mid 3]$ tras reducir ---
estado ``Sistema inconsistente $\cdot$ no tiene soluci\'on''.}
