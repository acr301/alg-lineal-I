...
from aqua_gauss.core.formato import MODO_FRACCION, latex_valor
...
def _fig_a_pixmap(fig, escala):
    ...
    imagen = QImage(bytes(canvas.buffer_rgba()), ancho, alto, QImage.Format.Format_RGBA8888).copy()
...
def latex_a_pixmap(expr, fontsize=15, color=COLOR_TEXTO, escala=2, pad=6):
    ...
    texto = fig.text(
        0.5, 0.5, f"${expr}$", fontsize=fontsize, color=color, ha="center", va="center"
    )
    canvas = FigureCanvasAgg(fig)
    canvas.draw()
...
def columna_a_pixmap(componentes, modo=MODO_FRACCION, fontsize=15, color=COLOR_TEXTO, escala=2):
    ...
    for i, token in enumerate(tokens):
        ax.text(
            0.5, n - i - 0.5, f"${token}$", ha="center", va="center", fontsize=fontsize, color=color
        )

    grosor = max(1.2, fontsize / 11.0)
    oreja = 0.12
    for x0, direccion in ((0.055, 1), (0.945, -1)):
        ax.plot([x0, x0], [0.06, n - 0.06], color=color, lw=grosor, solid_capstyle="round")
        ax.plot([x0, x0 + direccion * oreja], [0.06, 0.06], color=color, lw=grosor)
        ax.plot([x0, x0 + direccion * oreja], [n - 0.06, n - 0.06], color=color, lw=grosor)
